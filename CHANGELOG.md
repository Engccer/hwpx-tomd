...
### Fixed
- 새 값이 하나도 없는 행 — 모든 칸이 위 행 세로 병합(rowSpan)의 연속이거나 빈 시작 칸이고 연속 칸이 하나라도 있는 행 — 을 지운다. 레이아웃용 빈 열이 함께 있으면 그 칸은 연속이 아니라서 행이 살아남고 `--prune-empty`로 그 열이 지워진 뒤 똑같은 행만 남았기 때문에 빈 시작 칸까지 본다. `merge_fill="vertical"`이면 그런 행이 위 행과 똑같은 행으로 한 번 더 나왔고(단위학교 안내자료 교육활동 침해 처리 절차표의 단계 행 13개가 두 번씩), 채우지 않으면 빈 행이었다. GFM은 rowSpan을 표현하지 못하므로 그 행에는 남길 정보가 없다(실측: 2024 단위학교 근무지원 안내자료 3차 검수)
