...
### 3.5 자가검증(기존 정신 계승)
- 경고문에 배치 수(`count_images()` 반환값)와 고유 추출 수를 함께 표기한다: "본문 이미지 N개 배치(고유 M개) 추출 완료." BinData 누락 등으로 실제 삽입 참조가 더 적을 수 있으나, 3.4에 따라 깨진 링크는 생기지 않는다. 별도의 수치 대조 경고는 두지 않는다.
