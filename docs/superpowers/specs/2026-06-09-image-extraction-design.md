...
2. `image_dir`가 지정된 경우:
   1. 매니페스트(`Contents/content.hpf`)에서 `binaryItemIDRef → BinData/파일경로` 매핑을 읽는다. (실측상 ID `image1`이 `BinData/image1.*`와 stem 일치하므로, 매니페스트 파싱 실패 시 stem 매칭으로 폴백.)
...
| BinData 누락·깨짐 | 경고, 해당 항목 `file=null`(참조는 유지) |
| 같은 ID 중복 참조 | 파일 1회만 추출, 참조 공유 |
| 매니페스트 파싱 실패 | stem 매칭(`imageN` → `BinData/imageN.*`)으로 폴백 |
